<template>
  <h1 class="text-5xl font-bold mt-20">Équipe</h1>
  <br>
  <p class="text-lg mb-8">Rencontrez notre équipe talentueuse</p>
  <section class="grid grid-cols-3 gap-10">
    <div v-for="(membre, index) in membres" :key="membre.id"
      ...
      <img :src="membre.image" :alt="membre.nom" class="w-28 h-28 mt-8 rounded-full border-4 border-slate-50 z-10 group-hover:scale-150 group-hover:-translate-x-24 group-hover:-translate-y-20 transition-all duration-500">
      <div class="z-10 group-hover:-translate-y-10 transition-all duration-500">
        <span class="text-2xl font-semibold text-black-700">{{ membre.nom }}</span>
